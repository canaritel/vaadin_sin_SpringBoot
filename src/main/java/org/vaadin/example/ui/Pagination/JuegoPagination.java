...
public class JuegoPagination extends HorizontalLayout {
    ...
    public JuegoPagination() {
    }

    public JuegoPagination(JuegoService juegoService, Grid grid, TextField filterText) {
    ...
    private void datosPagination() {
        totalRegistros = juegoService.total(); //guardamos el total de registros
    ...
    private void createListener() {
        ...
        buttonInicio.addClickListener(e -> {
            if ((numeroPagina + 1) <= 1) {
                ...
                numeroPagina = 0;
                grid.setItems(juegoService.listarPagination(filterText.getValue(), false, itemsPagina, numeroPagina));
                buttonPagina.setText("Página " + (numeroPagina + 1) + " de " + totalPaginas);
            }
        });
