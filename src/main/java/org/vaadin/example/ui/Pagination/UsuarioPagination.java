...
public class UsuarioPagination extends HorizontalLayout {

    private UsuarioService usuarioService = new UsuarioService();
    private Grid<Usuario> grid = new Grid(Distribuye.class);
    ...
    public UsuarioPagination() {
    }

    public UsuarioPagination(UsuarioService usuarioService, Grid grid, TextField filterText) {
    ...
    private void datosPagination() {
        totalRegistros = usuarioService.total(); //guardamos el total de registros
    ...
    private void createListener() {
        ...
        buttonInicio.addClickListener(e -> {
            if ((numeroPagina + 1) <= 1) {
                ...
                numeroPagina = 0;
                grid.setItems(usuarioService.listarPagination(filterText.getValue(), false, itemsPagina, numeroPagina));
                buttonPagina.setText("Página " + (numeroPagina + 1) + " de " + totalPaginas);
            }
        });
